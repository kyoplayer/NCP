...
# それぞれのファイルについて
- `Problem.html` : 一般的な問題ファイルのテンプレートです。
- `Problem_Subtask.html` : 小課題制の問題に対応しているテンプレートです。
- `Editorial.html` : 一般的な解説ファイルのテンプレートです。

# 注記
- 2026/09/20 17:40にテンプレートを更新しました。
  これまで、seed値を、個別提出の欄と一括提出のための欄の両方に入れるために2回埋め込みを要求していましたが、一度でいいように変更しました。`<script>`タグの最上位に存在する、`seeds`にseed値を入力してください。個別提出の欄と、seed値が一覧で並んでいる部分を自動で生成されます。

- 2026/08/03日8:45にテンプレートを更新しました。
　パスワードロジックを全面的に差し替えました。コンテストのWriterで打ち合わせをし、コンテストを通して共通する秘密鍵を決定してください。復元コードは当リポジトリ内に存在します。
